...
void init_arrays(void)
{
	input        = (complex *) calloc(BUFLEN, sizeof(complex)); /* Input array */
    output       = (complex *) calloc(BUFLEN, sizeof(complex)); /* Output array */
    intermediate = (complex *) calloc(BUFLEN, sizeof(complex)); /* Array for processing*/
...
{       
	...
	input[index].r = ((float)sample)/scale;
		
	/* write new output data */
	mono_write_16Bit((short)(output[index].r*scale)); 
...
void wait_buffer(void)
{
	...
	p = (complex *) malloc(sizeof(complex)*BUFLEN);
	memcpy(p, intermediate, sizeof(complex)*BUFLEN);
	...
	ifft(BUFLEN, p);
	memcpy(intermediate, p, sizeof(complex)*BUFLEN);
	free (p);
