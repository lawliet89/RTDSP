...
that could enhance the filter are discussed in \secref{Further possible ideas}.
...
when evaluating the performance of the filterThe ``factory2'' corrupted
sample is used for evaluation of the filter. Figure \ref{fig:spec-clean}
...
noise estimation. As the filter is always pessimistic about the noise
level, the lower estimate before the transition is always dominating
until the relevant minimum noise buffer is discarded (around 7.5 seconds).
Due to the high over-subtraction factor $\alpha$ introduced in \eqref{oversubtraction}
to correct the underestimation of noise, some of the speech is also
removed and results in the voice being slurry. There is also a very
pronounced level of musical noise.
...
bin will potentially stay as the overly pessimistic noise estimate
...
optimism of the noise level. Furthermore, for the same reasons of
the filter being overly pessimistic, any noise with small spectral
...
One way of improving on the naive spectral subtractor is to low-pass
...
this would cause a sharp fall in $|N(\omega)|$ and cause a discontunity
...
Fourier Transform (FFT) used for DFT can be the most efficient. The
...
The filter has removed the musical noise present during perioids when
...
\section{\label{sec:Further possible ideas}Further ideas}
...
tracking the noise even during the connection setup delay period.
